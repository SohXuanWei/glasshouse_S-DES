...
int main ()
{
    ...
    outfile.open("testout.txt", std::ios::out | std::ios::trunc);

    std::string temp;
    std::vector<std::vector<int>> vec;

    //some practice at vector manipulation
    while (! infile.eof()) {
        ...
        for (int i = 0; i < temp.length(); i++) {
            ...
            if (c != ' ' && i != temp.length() -1) {
                ...
                tempVec.push_back(str_to_int(tempInt) +1);
                std::cout << str_to_int(tempInt) << " ";
                tempInt = "";
            }
        }
    ...
    //print out vector array vec to outfile
    for (int i = 0; i < vec.size(); ++i) {
        for (int j = 0; j < vec[i].size(); ++j) {
            outfile << vec[i][j] << " ";
        }
    ...
    */}
    ...
    ///read the input file
        ...
        //read the 8-bit plaintext and read the 10-bit key
        std::string ptxt; getline(rfile_in, ptxt);
    ...
    //IGNORE: first draft for permutation function
    {/*    std::string key_copy = key;
        key = "";

        while (rfile_p10 >> temp) {
            int i = str_to_int(temp);

            if (i <= (key_copy.length()-1)/2) { //considering the space separating two parts of the key, 01234=12345; 678910=678910; 5=space
                key = key + key_copy[i-1];
            }
            else {
                key = key + key_copy[i];
            }

            //adds a space at the middle of the key
            if (key.length() == (key_copy.length()-1)/2)
            {
                key = key + " ";
            }
        }*/}

    //run left shift 1 on the key
        //open up the ls_1 file
        std::ifstream rfile_ls1;
        rfile_ls1.open("./keygen/ls_1.txt", std::ios::in);
        //get rid of the first 3 lines of comments in the text file
        std::getline(rfile_ls1, temp); std::getline(rfile_ls1, temp); std::getline(rfile_ls1, temp);
        //get the permutation array in a vector
        while (rfile_ls1 >> temp) {
            px.push_back(str_to_int(temp));
        }
        key = rearrange(px,key);
        rfile_ls1.close(); px.clear();

        std::cout << "Left-Shift 1  : " << key << std::endl;

    //acquire key 1 by running p8 on the key
        ...
        std::string k1 = rearrange(vec_p8,key);
        rfile_p8.close();

        std::cout << "Key 1         :  " << k1 << std::endl;

    //run left-shift 2 on the key
        //open up the ls_2 file
        std::ifstream rfile_ls2;
        rfile_ls2.open("./keygen/ls_2.txt", std::ios::in);
        //get rid of the first 3 lines of comments in the text file
        std::getline(rfile_ls2, temp); std::getline(rfile_ls2, temp); std::getline(rfile_ls2, temp);
        //get the permutation array in a vector
        while (rfile_ls2 >> temp) {
            px.push_back(str_to_int(temp));
        }
        key = rearrange(px, key);
        rfile_ls2.close(); px.clear();

        std::cout << "Left-Shift 2  : " << key << std::endl;

    //acquire key 2 by running p8 on the key
        //reuse the p8 vector
        std::string k2 = rearrange(vec_p8,key);
        std::cout << "Key 2         :  " << k2 << std::endl;
    ...
    for (int x = 1; x < rounds; x++) {
        std::cout << std::endl << "==============" << std::endl << "| ENCRYPTION |" << std::endl << "==============" << std::endl;
        std::cout << "PLAINTEXT        : " << ptxt << std::endl;

        //run initial permutation on the plaintext
            //open up the IP file
            ...
            ptxt = rearrange(px, ptxt);
            rfile_ip.close(); px.clear();

            std::cout << "Init. Permutation: " << ptxt << std::endl;
        ...
        //run Expand and Permutate operation on the left side of the key
            ...
            lf_txt = rearrange(vec_ep, ptxt);
            rfile_ep.close();

            std::cout << std::endl << "-- Function Fk --" << std::endl;
            std::cout << "Expd. Permutation: " << lf_txt << std::endl;

        //run XOR operation with lf_txt and key 1 (k1)
            lf_txt = x_or(lf_txt, k1);
        ...
        //run xor operation with lf_txt and left halve of the plaintext
            lf_txt.erase(2,1);
            lf_txt = x_or(lf_txt, ptxt);

            std::cout << "XOR with Left Key: " << lf_txt << std::endl <<std::endl;

        //switch operation
            ptxt.erase(0,(ptxt.length()+1)/2);
            ptxt = ptxt + " " + lf_txt;

            std::cout << "Switch Halves    : " << ptxt << std::endl;
    }
    //final round
    ...
    //xor with key 2 (k2)
        lf_txt = x_or(lf_txt, k2);
        //set middle of string to empty space
        lf_txt[(lf_txt.length()-1)/2] =  ' ';

        std::cout << "XOR with Key 2   : " << lf_txt << std::endl;
    ...
    //no need for switching, combine plaintext and run inverse IP (IP-1)
        ...
        std::cout << "L+R combined     : " << ptxt << std::endl;
        //run inverse IP (IP-1)
        ...
        std::cout << "Inv. Permutation : " << ptxt << std::endl;
