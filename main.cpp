...
int main ()
{
    ...
    */}
    ...
    for (int x = 1; x < rounds; x++) {
        ...
        //run XOR operation with lf_txt and key x-1 (vector k)
            lf_txt = x_or(lf_txt, k[x-1]);
            //set middle of string to empty space
            lf_txt[(lf_txt.length()-1)/2] =  ' ';

            std::cout << "XOR with Key " << x <<  "   : " << lf_txt << std::endl;
    ...
    //xor with key 2 (k2)
        lf_txt = x_or(lf_txt, k.back());
        //set middle of string to empty space
        lf_txt[(lf_txt.length()-1)/2] =  ' ';

        std::cout << "XOR with Key " << k.size() <<"   : " << lf_txt << std::endl;
